...
BEGIN_MESSAGE_MAP(CClassView, CDockablePane)
	...
	ON_BN_CLICKED(902, OnNewFolder)
	ON_WM_PAINT()
	ON_WM_SETFOCUS()
	ON_COMMAND_RANGE(ID_SORTING_GROUPBYTYPE, ID_SORTING_SORTBYACCESS, OnSort)
	ON_UPDATE_COMMAND_UI_RANGE(ID_SORTING_GROUPBYTYPE, ID_SORTING_SORTBYACCESS, OnUpdateSort)
	ON_EN_CHANGE(1001, OnSearchBoxChange)
END_MESSAGE_MAP()
...
int CClassView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	SetWindowLongPtr(GetSafeHwnd(), DWLP_MSGRESULT, CDRF_NOTIFYITEMDRAW);

	CRect rectDummy;
	rectDummy.SetRectEmpty();

	// Create views:
	const DWORD dwViewStyle = WS_CHILD | WS_VISIBLE | WS_BORDER | TVS_HASLINES | TVS_LINESATROOT | TVS_HASBUTTONS | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | TVS_SHOWSELALWAYS;

	if (!m_wndClassView.Create(dwViewStyle, rectDummy, this, 902))
	{
	...
	m_loc.Create(WS_VISIBLE | WS_CHILD | WS_BORDER, CRect(), this, 1002);
	m_loc.SetFont(&afxGlobalData.fontRegular);
	m_loc.SetWindowText(_T("12.218, 2.55, 121.71"));
	
	// m_loc.EnablePreview(-100.0f, 100.0f, -100.0f, 100.0f);
...
void CClassView::AdjustLayout()
{
	...
	const int searchBarHeight = static_cast<int>(afxGlobalData.GetRibbonImageScale() * 20.0);
	m_search.SetWindowPos(nullptr, rectClient.left + 1, rectClient.top + cyTlb + 1, rectClient.Width() - 2, searchBarHeight, SWP_NOACTIVATE | SWP_NOZORDER);
	
	const int bottomAreaHeight = static_cast<int>(afxGlobalData.GetRibbonImageScale() * 60.0);
	const int bottomAreaSpacing = static_cast<int>(afxGlobalData.GetRibbonImageScale() * 10.0);
	const int vectorEditBoxHeight = static_cast<int>(afxGlobalData.GetRibbonImageScale() * 32.0);
	m_wndClassView.SetWindowPos(nullptr, rectClient.left + 1, rectClient.top + cyTlb + searchBarHeight + 1, rectClient.Width() - 2, rectClient.Height() - cyTlb - searchBarHeight - bottomAreaHeight - 2, SWP_NOACTIVATE | SWP_NOZORDER);

	m_loc.MoveWindow(rectClient.left + 1, rectClient.bottom - bottomAreaHeight + bottomAreaSpacing, rectClient.Width() - 2, vectorEditBoxHeight);
	m_loc.Redraw();
...
void CClassView::OnSearchBoxChange()
{
	CString tokens;
	m_search.GetWindowText(tokens);
	m_wndClassView.SetHighlightTokens(tokens);
}
...
void CClassView::OnNewFolder()
{
	...
	HTREEITEM hClass = m_wndClassView.InsertItem(_T("New Folder"), 2, 2, hRoot);
	m_wndClassView.InsertItem(_T("CNewRandomClass"), 5, 5, hClass);
	m_wndClassView.Expand(hClass, TVE_EXPAND);
}

void CClassView::OnRemoveAll()
{
